--ApplyGTEMod(GTE,Mass,Drag,FWD,RWD,UpShift,DownShift,Drive,MaxFlat,InitMaxFlat,Brake,Steering,CurveMax,CurveMaxR,CurveMin,CurveMinR,CurveLat,CurveLatR,Coll,Eng,DF)
if CarNameCurrent=='SPECTER2' then
	 local Mass = 0
	 local Drag = 0
	 local FWD = 0
	 local RWD = 0
	 local UpShift = 3
	 local DownShift = 3
	 local Drive = 0.01
	 local MaxFlat = 3
	 local InitMaxFlat = 0
	 local Brake = 0
	 local Steering = -0.1
	 local CurveMax = -0.17 --2.67
	 local CurveMaxR = 0 --0.3745
	 local CurveMin = -0.003 --2.55
	 local CurveMinR = -0.4 --8.33
	 local CurveLat = 0 --0.3926
	 local CurveLatR = 0 --2.5464
	 local Coll=10
	 local Eng=2.5
	 local DF=-1.75
     ApplyGTEMod(GTE,Mass,Drag,FWD,RWD,UpShift,DownShift,Drive,MaxFlat,InitMaxFlat,Brake,Steering,CurveMax,CurveMaxR,CurveMin,CurveMinR,CurveLat,CurveLatR,Coll,Eng,DF)
 elseif CarNameCurrent=='COQUETTE4' then
	 local Mass = 0
	 local Drag = 0
	 local FWD = 0
	 local RWD = 0
	 local UpShift = 3.4
	 local DownShift = 3.1
	 local Drive = 0
	 local MaxFlat = 0
	 local InitMaxFlat = 0
	 local Brake = 0
	 local Steering = 0
	 local CurveMax = -0.09 --2.6
	 local CurveMaxR = 0 --0.3846
	 local CurveMin = 0 --2.34
	 local CurveMinR = 0 --4
	 local CurveLat = 0--0.3577
	 local CurveLatR = 0--2.7949
	 local Coll=10
	 local Eng=2.5
	 local DF=0
     ApplyGTEMod(GTE,Mass,Drag,FWD,RWD,UpShift,DownShift,Drive,MaxFlat,InitMaxFlat,Brake,Steering,CurveMax,CurveMaxR,CurveMin,CurveMinR,CurveLat,CurveLatR,Coll,Eng,DF)
 elseif CarNameCurrent=='COMET6' then
	 local Mass = 0
	 local Drag = 0
	 local FWD = 0
	 local RWD = 0
	 local UpShift = 2.8
	 local DownShift = 2.8
	 local Drive = -0.01
	 local MaxFlat = -1
	 local InitMaxFlat = 0
	 local Brake = 0
	 local Steering = 0
	 local CurveMax = 0.045 --2.65
	 local CurveMaxR = 0.008 --0.3766
	 local CurveMin = -0.09 --2.52
	 local CurveMinR = -1.7037 --7.4074
	 local CurveLat = -0.0131 --0.3839
	 local CurveLatR = 0.09545 --2.604
	 local Coll=10
	 local Eng=2.5
	 local DF=-120
     ApplyGTEMod(GTE,Mass,Drag,FWD,RWD,UpShift,DownShift,Drive,MaxFlat,InitMaxFlat,Brake,Steering,CurveMax,CurveMaxR,CurveMin,CurveMinR,CurveLat,CurveLatR,Coll,Eng,DF)
 elseif CarNameCurrent=='ITALIRSX' then
	 local Mass = -600
	 local Drag = 0
	 local FWD = -1
	 local RWD = 0
	 local UpShift = -1
	 local DownShift = -1
	 local Drive = -0.0402
	 local MaxFlat = -1.223
	 local InitMaxFlat = -1.4394
	 local Brake = -0.3
	 local Steering = -0.05
	 local CurveMax = -0.127
	 local CurveMaxR = 0.0268
	 local CurveMin = 0.69
	 local CurveMinR = -2.1667
	 local CurveLat = -0.0262
	 local CurveLatR = 0.1367
	 local Coll=10
	 local Eng=2.5
	 local DF=-350
     ApplyGTEMod(GTE,Mass,Drag,FWD,RWD,UpShift,DownShift,Drive,MaxFlat,InitMaxFlat,Brake,Steering,CurveMax,CurveMaxR,CurveMin,CurveMinR,CurveLat,CurveLatR,Coll,Eng,DF)
 elseif CarNameCurrent=='CYPHER' then
	 local Mass = 0
	 local Drag = 0
	 local FWD = 0
	 local RWD = 0
	 local UpShift = -0.8
	 local DownShift = -0.8
	 local Drive = 0.075
	 local MaxFlat = 0.5
	 local InitMaxFlat = 0
	 local Brake = 0
	 local Steering = -0.045
	 local CurveMax = -0.18 --2.59
	 local CurveMaxR = 0 --0.3861
	 local CurveMin = -0.1 --2.46
	 local CurveMinR = 0 --7.6923
	 local CurveLat = 0 --0.3926
	 local CurveLatR = 0 --2.5464
	 local Coll=10
	 local Eng=2.5
	 local DF=0
     ApplyGTEMod(GTE,Mass,Drag,FWD,RWD,UpShift,DownShift,Drive,MaxFlat,InitMaxFlat,Brake,Steering,CurveMax,CurveMaxR,CurveMin,CurveMinR,CurveLat,CurveLatR,Coll,Eng,DF)

end

--[LOG: Brakes for all cars -0.4]
--[LOG: Brakes for all cars +0.4]
--[LOG: Added Vectre and Growler]
